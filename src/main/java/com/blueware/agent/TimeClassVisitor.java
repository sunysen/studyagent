...
public class TimeClassVisitor extends ClassVisitor {
    ...
    @Override public MethodVisitor visitMethod(int access, final String name, String desc, String signature, String[] exceptions) {
        ...
        if (!name.equals("<init>") && mv != null) {
            mv = new AdviceAdapter(Opcodes.ASM5, mv, access, name, desc) {
                ...
                @Override public void onMethodEnter() {
                    ...
                    this.visitMethodInsn(Opcodes.INVOKESTATIC, "com/blueware/agent/TimeUtil", "setStartTime", "()V", false);
                }

                //方法退出时获取结束时间并计算执行时间
                @Override public void onMethodExit(int opcode) {
                    //相当于com.blueware.agent.TimeUtil.setEndTime();
                    this.visitMethodInsn(Opcodes.INVOKESTATIC, "com/blueware/agent/TimeUtil", "setEndTime", "()V", false);
                    //向栈中压入类名称
                    this.visitLdcInsn(className);
                    //向栈中压入方法名
                    this.visitLdcInsn(name);
                    //相当于com.blueware.agent.TimeUtil.getExclusiveTime("com/blueware/agent/TestTime","testTime");
                    this.visitMethodInsn(Opcodes.INVOKESTATIC, "com/blueware/agent/TimeUtil", "getExclusiveTime", "(Ljava/lang/String;Ljava/lang/String;)J", false);
                }
            };
        }
        return mv;
